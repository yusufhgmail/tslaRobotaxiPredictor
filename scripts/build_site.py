...
<meta name="description" content="Daily-updated forecast of Tesla's unsupervised robotaxi fleet vs. the 1,800-vehicle re-rating threshold." />
...
(() => {
  ...
  // Linear-mode Y cap = top of the P5–P95 fuzzy band (with a small headroom),
  // so the wide tail of high-end Monte Carlo trajectories doesn't squash the
  // visible band against the bottom of the chart.
  const p95Max = (data.p95 || []).reduce((m, v) => v > m ? v : m, 0);
  const histMax = histY.reduce((m, v) => v > m ? v : m, 0);
  const linearYMax = Math.max(p95Max, histMax) * 1.05;
  function setScale(mode) {
    ...
    if (mode === 'linear' && linearYMax > 0) {
      update['yaxis.autorange'] = false;
      update['yaxis.range'] = [0, linearYMax];
    } else {
